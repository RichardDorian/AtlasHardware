...
// If the post is already saved by the user, return a 409 Conflict status code and exit the script
if ($result->num_rows === 1) {
  http_response_code(409);
  exit();
}

// Prepare and execute a query to insert the new saved post into the database
$stmt = $link->prepare("INSERT INTO users_saved_posts (user, post) VALUES (unhex(?), unhex(?))");
